...
/* ===== 聊天主流程 ===== */
/* ===== 微信风格输入栏与面板交互 ===== */
let inputVoiceMode = false;

function toggleInputVoiceMode(){
  inputVoiceMode = !inputVoiceMode;
...
function handleInputChange(){
  ...
  if(!msgInput || !actionBtn || !sendBtn) return;
  
  const val = msgInput.value.trim();
  if(val.length > 0 || pendingImage){
...
function toggleEmojiPanel(){
  ...
  if(emoji) {
    ...
    if(emoji.classList.contains('show')) initEmojiGrid();
  }
}
...
function insertEmoji(e){
  ...
  if(!msgInput) return;
  msgInput.value += e;
  if(typeof autoResize === 'function') autoResize(msgInput);
  handleInputChange();
  msgInput.focus();
}

function autoResize(el){el.style.height='auto';el.style.height=Math.min(el.scrollHeight,90)+'px';if(tokenPanelOpen&&document.getElementById('tokenPanel').classList.contains('show'))renderTokenBody();}
function triggerFileInput(id){document.getElementById(id).click();if(id!=='genInitInput')closeExtraPanels();}
...
async function sendMessage(){
  ...
  try {
    ...
    if (typeof closeExtraPanels === 'function') closeExtraPanels();
...
async function sendAudioMessage(displayText, base64Data, mimeType, transcript='', durationSec=1){
  if(chatReplying) return;
  chatReplying = true;
  try {
    if (typeof applyRelationshipDecay === 'function') { try { applyRelationshipDecay(); } catch (e) {} }
    if (typeof markActivity === 'function') { try { markActivity(); } catch (e) {} }
    if (typeof triggerHaptic === 'function') { try { triggerHaptic('medium'); } catch (e) {} }

    const cleanText = displayText ? displayText.replace(/^🎤\s*/, '') : '语音消息';
    const emotionObj = (typeof MultimodalAudio !== 'undefined' && MultimodalAudio.detectEmotion)
      ? MultimodalAudio.detectEmotion(transcript || cleanText)
      : { mood: 'calm', confidence: 0.8 };
    const emotion = emotionObj.mood || 'calm';
    ...
    // 音频数据存入 IndexedDB（通过 StorageManager），聊天记录只存元数据
    if (typeof StorageManager !== 'undefined' && base64Data) {
      try {
        ...
        console.warn('[sendAudioMessage] StorageManager.saveAudio failed:', e);
      }
    }

    // 旧 AudioStorageDB 降级保留（兼容）
    if (typeof AudioStorageDB !== 'undefined' && base64Data && typeof StorageManager === 'undefined') {
      try {
        ...
        console.warn('[sendAudioMessage] AudioStorageDB.saveAudio failed:', e);
      }
    }

    // 传给 requestAI 的音频对象（含 base64，本轮使用）
    const audioObj = {
      audioId: audioId,
      base64: base64Data,
      mimeType: mimeType || 'audio/webm',
    ...
    // 存聊天记录时只保留元数据，不含 base64（避免 localStorage 溢出）
    const audioMeta = {
      audioId: audioId,
      mimeType: mimeType || 'audio/webm',
      format: (typeof extForMime === 'function') ? extForMime(mimeType) : 'webm',
    ...
    renderTextMessage('user', cleanText, uid, null, null, false, ts, audioObj);
    saveHistory();

    const memoryContent = transcript || cleanText;
    if (memoryContent) {
      if (typeof memorize === 'function') { try { memorize('user', memoryContent, emotion); } catch (e) {} }
      if (typeof updateRelationshipState === 'function') { try { updateRelationshipState({ userEmotion: emotion, text: memoryContent }); } catch (e) {} }
      if (typeof recordLifeRhythmSignal === 'function') { try { recordLifeRhythmSignal(memoryContent, emotion); } catch (e) {} }
    }
...
let currentUserAudioUid = null;

function userVoiceBadgeHTML(uid, duration){
  const durStr = duration ? `${duration}″` : '1″';
  const svgIcon = `<svg width="14" height="14" viewBox="0 0 16 16" fill="currentColor" style="display:inline-block;vertical-align:-2px;"><circle cx="3" cy="8" r="1.5"/><path d="M6 4.5 C7.5 6, 7.5 10, 6 11.5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" fill="none"/><path d="M9.5 2.5 C12 5.5, 12 10.5, 9.5 13.5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" fill="none"/></svg>`;
  return `<span class="user-voice-badge" id="voice-badge-${uid}" onclick="event.stopPropagation(); playUserVoice('${uid}')" title="点击播放原声">${svgIcon}<span class="voice-dur">${durStr}</span></span>`;
}

async function playUserVoice(uid){
  const m = getMsg(uid);
  if(!m || !m.audio) {
    if (typeof showToast === 'function') showToast('未找到语音数据');
    return;
  }
  ...
  const badge = document.getElementById(`voice-badge-${uid}`);
  if(currentUserAudioUid === uid && currentUserAudio && !currentUserAudio.paused){
    currentUserAudio.pause();
    currentUserAudio = null;
    currentUserAudioUid = null;
    if(badge) badge.classList.remove('playing');
    return;
  }
  if(currentUserAudio){
    currentUserAudio.pause();
    const oldBadge = document.getElementById(`voice-badge-${currentUserAudioUid}`);
    if(oldBadge) oldBadge.classList.remove('playing');
    currentUserAudio = null;
    currentUserAudioUid = null;
  }
  try {
    ...
    currentUserAudio.onerror = (e) => {
      console.error('[UserVoice] Playback error:', e);
      if(badge) badge.classList.remove('playing');
      currentUserAudio = null;
      currentUserAudioUid = null;
      if(typeof showToast === 'function') showToast('语音播放失败');
    };
    currentUserAudio.play();
  } catch(e) {
    console.error('[UserVoice] Exception:', e);
    if(badge) badge.classList.remove('playing');
...
function renderTextMessage(role,content,uid,reasoning,recallItems,proactive,ts,audio){
  ...
  if(role!=='user'&&last){
    last.insertAdjacentHTML('beforeend',speakerHTML(uid));
  } else if(role==='user'&&last){
    const audioData = audio || (getMsg(uid) && getMsg(uid).audio);
    if(audioData){
      const dur = audioData.duration || 1;
      last.insertAdjacentHTML('beforeend', userVoiceBadgeHTML(uid, dur));
    }
  }
...
function rerenderAll(){const c=document.getElementById('chatMessages');c.innerHTML='';conversationHistory.forEach(m=>{if(m.image)renderImageMessage(m.role==='user'?'user':'assistant',m.image,m.uid,m.ts);else renderTextMessage(m.role==='imported'?'assistant':m.role,m.content,m.uid,m.reasoning,m.recallItems,m.proactive,m.ts,m.audio);});}
...
if(!endpoint){
  addMessage('assistant','❌ 服务商 Endpoint 未配置，请在设置中配置服务商地址',genUid());
  return;
}
...
if(!url.includes('/chat/completions')&&!url.includes('messages')&&!url.includes('/v1/chat')) {
...
if(currentImage){
  if(messages.length > 0 && messages[messages.length - 1].role === 'user'){
    messages[messages.length - 1].content = [
      {type:'text',text:queryText||'请描述这张图片'},
      {type:'image_url',image_url:{url:currentImage}}
    ];
  } else {
    messages.push({role:'user',content:[{type:'text',text:queryText||'请描述这张图片'},{type:'image_url',image_url:{url:currentImage}}]});
  }
  ...
  if (isGoogle) {
    ...
    const audioContent = [
      { type: 'text', text: audioPromptText },
      { type: 'inline_data', mime_type: audioMime, data: currentAudio.base64 }
    ];
    if(messages.length > 0 && messages[messages.length - 1].role === 'user'){
    ...
    // OpenAI audio-preview models: send audio as input_audio
...
const body={model:useModel,messages,stream};
if(localStorage.getItem('temp_enabled')==='true')body.temperature=parseFloat(localStorage.getItem('temperature')||'1');
if(localStorage.getItem('top_p_enabled')==='true')body.top_p=parseFloat(localStorage.getItem('top_p')||'1');
const requestKey=makeChatRequestKey(provider,useModel,body,url);
if(chatRequestInFlightKey===requestKey){
  console.warn('[API Dedup] Duplicate chat completion blocked.');
  showToast('已拦截重复 API 请求');
  return;
}
chatRequestInFlightKey=requestKey;
if(window.recordTokenTelemetry)recordTokenTelemetry({caller:'requestAI-input',provider:provider.id||provider.name||'',model:useModel,messages,promptChars:JSON.stringify(messages||[]).length,meta:{stream,hasImage:!!currentImage}});
    ...
    try{
      ...
      console.warn('[Streaming fetch failed, attempting non-stream fallback]:', err);
      if(!started){
        try{
          const fallbackBody = { ...body, stream: false };
          const r2 = await fetch(url, { method: 'POST', headers, body: JSON.stringify(fallbackBody) });
          if(!r2.ok) throw new Error(`API 错误 (${r2.status})`);
          const d2 = await r2.json();
          const m = d2.choices?.[0]?.message;
          full = m?.content || d2.content?.[0]?.text || '无响应';
...
function saveHistory(){
  ...
  const cleanHistory = conversationHistory.map(m => {
    if (m.audio && (m.audio.base64 || m.audio.audioId)) {
      const audioId = m.audio.audioId || `audio_msg_${m.uid || genUid()}`;
      m.audio.audioId = audioId;
      if (m.audio.base64 && typeof AudioStorageDB !== 'undefined') {
        AudioStorageDB.saveAudio(audioId, {
          base64: m.audio.base64,
  ...
  // 全量历史始终写入 IndexedDB（容量大，不受 localStorage 配额影响，绝不因溢出丢数据）
  ...
  try{
    ...
    // localStorage 放不下时不砍历史：完整数据已在 IndexedDB，本地只降级为一个截断缓存
    // 1) 先去图片 base64 再试（保留更多可见内容）
    ...
    try {
      ...
      // 2) 还不行就只缓存最近 200 条，内存 conversationHistory 与 IndexedDB 保持全量
      try {
        localStorage.setItem(key, JSON.stringify(cleanHistory.slice(-200)));
      } catch(eee) {
        console.warn('[SaveHistory] localStorage cache failed entirely:', eee.name);
      }
    }
  }
}
async function loadHistory(){
  ...
  const renderList = (history) => {
    ...
    history.forEach(m=>{
      ...
      else renderTextMessage(m.role==='imported'?'assistant':m.role,m.content,m.uid,m.reasoning,m.recallItems,m.proactive,m.ts,m.audio);
    });
  };
...
function clearChat(){if(confirm('确定清空当前所有对话？(不影响长期记忆)')){
  conversationHistory=[];
  const id=currentPrivateAiId();
  const key=id==='main'?'chatHistory':`chatHistory_${id}`;
  localStorage.removeItem(key);
  if (typeof HistoryBackupDB !== 'undefined') {
    HistoryBackupDB.set(key, []);
  }
