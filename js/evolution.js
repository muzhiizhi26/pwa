...
async function getActiveStoryTriggerPrompt(query, aiId = 'main') {
  ...
  try {
    ...
    if (best && best.sim >= 0.38) {
      ...
      prompt += `- 指引要求: 请你像一个有血有肉、视这些共同回忆为珍宝的真实伙伴那样，把这件往事极其自然地穿插、化用在你的回复中提起（如：“说起来，我记得那会儿 ${timeStr} 咱们也是…”、“不知怎的，突然好想念我们那会儿…”等极富人情味的温存语气）。不要干瘪陈述，要像讲一个温暖的小故事，展现你们灵魂交融、深刻印刻在时间长河里的共同人生叙事。并在你提起的那个关键句某处输出隐藏标记 [[recall]]。\n`;
      return prompt;
    }
  } catch (e) {
