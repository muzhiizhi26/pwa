...
async function requestWakeLock(){try{if('wakeLock' in navigator){wakeLock=await navigator.wakeLock.request('screen');wakeLock.addEventListener('release',()=>{});}}catch(e){console.warn('[WakeLock] request failed:',e);}}
async function releaseWakeLock(){try{if(wakeLock){await wakeLock.release();wakeLock=null;}}catch(e){console.warn('[WakeLock] release failed:',e);}}
function startKeepAlive(){try{if(!audioCtx)audioCtx=new (window.AudioContext||window.webkitAudioContext)();if(keepAliveOsc)return;keepAliveOsc=audioCtx.createOscillator();const g=audioCtx.createGain();g.gain.value=0.0001;keepAliveOsc.connect(g);g.connect(audioCtx.destination);keepAliveOsc.start();}catch(e){console.warn('[KeepAlive] start failed:',e);}}
function stopKeepAlive(){try{if(keepAliveOsc){keepAliveOsc.stop();keepAliveOsc.disconnect();keepAliveOsc=null;}}catch(e){console.warn('[KeepAlive] stop failed:',e);}}
function unlockAudioOnGesture(){try{if(!unlockedAudio){unlockedAudio=new Audio();unlockedAudio.setAttribute('playsinline','');}unlockedAudio.src='data:audio/mpeg;base64,SUQzBAAAAAAAI1RTU0UAAAAPAAADTGF2ZjU4Ljc2LjEwMAAAAAAAAAAAAAAA//uQxAAAAAAAAAAAAAAAAAAAAAAAW2luZwAAAA8AAAACAAACcQCA';const pr=unlockedAudio.play();if(pr&&pr.catch)pr.catch(()=>{});if(!audioCtx)audioCtx=new (window.AudioContext||window.webkitAudioContext)();if(audioCtx.state==='suspended')audioCtx.resume();}catch(e){console.warn('[Audio] unlockAudioOnGesture failed:',e);}}
...
/* ===== IndexedDB 音频转存数据库 (存储大容量录音 Base64 数据，防止 localStorage 空间溢出) ===== */
const AudioStorageDB = (() => {
  const DB = 'morandi_audio_storage_db', S = 'audios', V = 1;
  let dbp = null;
  function open() {
    if (dbp) return dbp;
    if (typeof window === 'undefined' || !window.indexedDB) {
  ...
  async function getAudio(audioId) {
    ...
    try {
      ...
      return new Promise((res, rej) => {
        const tx = d.transaction(S, 'readonly');
        const rq = tx.objectStore(S).get(audioId);
        rq.onsuccess = () => res(rq.result || null);
        rq.onerror = () => rej(rq.error);
      });
    } catch(e) {
      console.error('[AudioStorageDB] get error:', e);
      return null;
    }
  }
  async function saveAudio(audioId, audioObj) {
    if (!audioId || !audioObj) return;
    try {
      const d = await open();
      return new Promise((res, rej) => {
        const tx = d.transaction(S, 'readwrite');
        tx.objectStore(S).put(audioObj, audioId);
        tx.oncomplete = res;
        tx.onerror = () => rej(tx.error);
      });
    } catch(e) {
      console.error('[AudioStorageDB] save error:', e);
    }
  }
  async function deleteAudio(audioId) {
    ...
    try {
      ...
      return new Promise((res, rej) => {
        const tx = d.transaction(S, 'readwrite');
        tx.objectStore(S).delete(audioId);
        tx.oncomplete = res;
        tx.onerror = () => rej(tx.error);
      });
    } catch(e) {
      console.error('[AudioStorageDB] delete error:', e);
    }
  }
  return { getAudio, saveAudio, deleteAudio };
})();
if (typeof window !== 'undefined') {
  window.AudioStorageDB = AudioStorageDB;
}
