/* ===== 服务商核心 ===== */
function loadProviders(){let s=localStorage.getItem('ai_providers');if(s){try{providers=JSON.parse(s);}catch(e){providers=[];}}if(!providers.length){providers=JSON.parse(JSON.stringify(DEFAULT_PROVIDERS));}providers=providers.filter(p=>p.id!=='openai'&&p.id!=='claude'&&p.id!=='gemini_proxy');const vi=providers.findIndex(p=>p.id==='vector');if(vi>0){const [vec]=providers.splice(vi,1);providers.unshift(vec);}else if(vi===-1){const vec=DEFAULT_PROVIDERS.find(p=>p.id==='vector');if(vec)providers.unshift(JSON.parse(JSON.stringify(vec)));}const fi=providers.findIndex(p=>p.id==='free');if(fi>=0)providers[fi]=JSON.parse(JSON.stringify(FREE_PROVIDER));else providers.splice(1, 0, JSON.parse(JSON.stringify(FREE_PROVIDER)));saveProviders();}
function saveProviders(){localStorage.setItem('ai_providers',JSON.stringify(providers));}
function loadSettings(){currentProviderId=localStorage.getItem('current_provider')||'vector';selectedModelName=localStorage.getItem('selected_model')||'';let p=getProvider(currentProviderId);if(!p){currentProviderId=providers[0]?.id||'vector';p=getProvider(currentProviderId);selectedModelName='';}if(p?.models.length){const hasModel=p.models.some(m=>m.name===selectedModelName);if(!hasModel||!selectedModelName){selectedModelName=p.models[0].name;saveSettings();}}}
...
async function compressConversation(silent){
  ...
  function localSummary(){
    ...
    const userMsgs=msgs.filter(m=>m.role==='user').slice(-5).map(m=>m.content).join('；');
    const aiMsgs=msgs.filter(m=>m.role==='assistant').slice(-3).map(m=>m.content.slice(0,50)).join('；');
    const ts=new Date();
...
function renderGeneralSettings(){settingsMode='general';document.getElementById('detailTitle').innerHTML='⚙️ 通用设置';const fs=localStorage.getItem('font_size')||'15';const te=localStorage.getItem('temp_enabled')==='true',tv=localStorage.getItem('temperature')||'1';const pe=localStorage.getItem('top_p_enabled')==='true',pv=localStorage.getItem('top_p')||'1';const bg=localStorage.getItem('chat_bg');const cl=getContextLimit();const contextLimitText=cl===Infinity?'不限制':cl+' 条';const contextLimitSliderValue=cl===Infinity?200:Math.min(cl,200);document.getElementById('detailBody').innerHTML=`
    ...
    <div class="action-buttons" style="margin-top:16px;"><button class="btn btn-warning" onclick="exportChat()">📥 导出记录</button><button class="btn btn-danger" onclick="clearChat()">🗑️ 清对话</button><button class="btn btn-info" onclick="deepCleanStorage()">🧹 深度清理存储</button><button class="btn btn-info" onclick="resetProviders()">⟳ 重置服务商</button></div>
...
async function clearVectorMemory(){if(!confirm('确定清空所有长期记忆？（包含所有已保存记忆事实，此操作不可恢复）'))return;await VDB.clear();localStorage.removeItem('memory_events_log');localStorage.removeItem('mid_term_summary');localStorage.removeItem('long_term_profile');if(typeof showToast==='function')showToast('✅ 已清空所有长期记忆');if(typeof renderMemorySettings==='function')renderMemorySettings();if(typeof refreshTabContent==='function')refreshTabContent();}
...
function addProvider(){const n=prompt('输入服务商名称');if(!n)return;const id='custom_'+Date.now();providers.push({id,name:n,icon:'⚙️',endpoint:'',auth:'Bearer',models:[],note:'自定义'});saveProviders();selectProvider(id);showToast('✅ 已添加');}
...
function resetProviders(){if(confirm('重置所有服务商为默认？')){providers=JSON.parse(JSON.stringify(DEFAULT_PROVIDERS));saveProviders();currentProviderId='vector';selectedModelName=providers[0]?.models[0]?.name||'';saveSettings();renderProviderList();updateModelCard();}}
