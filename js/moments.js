...
2. 不要生硬带引号、Markdown或AI词汇。`;
  ...
  async userPublishMoment(text, imageBase64) {
    ...
    const mainAiName = localStorage.getItem('ai_name') || '小艾';
    ...
    if (typeof getGroupMembers !== 'function' && window.LazyLoader) {
      try { await window.LazyLoader.load('js/group.js?v=20260807-group-budget').catch(() => {}); } catch(e) {}
    }
  ...
  addCommentToMoment(momentId, authorType, name, text, replyToId = null, replyToName = null) {
    ...
    if (authorType === 'user') {
      let aiId = replyToId || moment.ai_id || 'main';
      if (aiId === 'user') {
        aiId = moment.ai_id || 'main';
      }
      
      let aiName = localStorage.getItem('ai_name') || '小艾';
      if (aiId !== 'main' && typeof memberById === 'function') {
        const mem = memberById(aiId);
        if (mem) {
          aiName = mem.name || aiName;
        }
      }
  ...
  renderMomentsTab() {
    ...
    if (typeof getGroupMembers !== 'function' && window.LazyLoader) {
      window.LazyLoader.load('js/group.js?v=20260807-group-budget').then(() => {
            ...
            commentsPartHtml = `
              <div class="wechat-comments-list" style="display: flex; flex-direction: column; gap: 4px; padding: 4px 0; border-top: ${hasLikes ? '0.5px solid #e2e2e2' : 'none'}; margin-top: ${hasLikes ? '4px' : '0'};">
                ${m.comments.map(c => {
                  ...
                  } else if (c.author === 'main' || c.author === 'ai') {
                    commentatorName = aiName;
                  } else if (typeof memberById === 'function' && c.author) {
                    const mem = memberById(c.author);
                    if (mem) {
                      commentatorName = mem.name || c.name;
                    } else {
                      commentatorName = c.name || 'AI伴侣';
                    }
                  } else {
                    commentatorName = c.name || 'AI伴侣';
                  }
