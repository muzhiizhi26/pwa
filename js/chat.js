...
function renderImageMessage(role,src,uid,ts){const div=document.createElement('div');div.className=`message ${role==='user'?'user-message':'ai-message'}`;div.id='msg-'+uid;div.dataset.uid=uid;div.dataset.role=role;div.innerHTML=`<input type="checkbox" class="msg-check" onchange="onCheck('${uid}',this.checked)">${avatarHTML(role)}<div class="msg-content"><div class="bubbles"><div class="bubble image-bubble" onclick="toggleMsgActions('${uid}'); event.stopPropagation();"><img src="${src}" onclick="event.stopPropagation(); openImageViewer(this.src)"></div></div><div class="msg-time">${nowTime(ts)}</div><div class="msg-actions"><button onclick="copyAnchor('${uid}')" title="复制锚点">🔗</button><button onclick="msgDelete('${uid}')">🗑️</button></div></div>`;document.getElementById('chatMessages').appendChild(div);scrollBottom();}
