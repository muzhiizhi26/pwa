...
const MomentsEngine = {
  ...
  saveMoments(moments) {
    ...
    if (typeof HistoryBackupDB !== 'undefined') {
      HistoryBackupDB.set('lovestory_moments_backup', moments).catch(e => console.error('[Moments] Dual-write backup failed:', e));
    }
  },
...
刚才在朋友圈下面发生了一段对话：
动态内容: “${moment.content}”
用户针对了 ${replyToName} 的评论，回复道: “${text}”
...
2. 仅返回纯回复正文，不要有任何前言、后记、Markdown 或引号。`;
          ...
          this.addCommentToMoment(momentId, 'ai', aiName, aiReplyFallback, 'user', userName);
        }
      }, 2000);
    }
  },
  ...
  renderMomentsTab() {
    ...
    const moments = this.getMoments();
    const momentsBg = localStorage.getItem('moments_bg_image') || 'https://images.unsplash.com/photo-1505691938895-1758d7feb511?auto=format&fit=crop&w=1000&q=80';
    ...
    container.innerHTML = `
      ...
      <div class="moments-hero" onclick="document.getElementById('momentsHeroInput').click()" style="background-image: url('${momentsBg}'); background-size: cover; background-position: center; height: 240px; position: relative; margin-bottom: 28px; box-shadow: inset 0 -40px 60px rgba(0,0,0,0.3); cursor: pointer;" title="点击更换背景大图">
  ...
  handleHeroImage(input) {
    ...
    const r = new FileReader();
    r.onload = e => {
      localStorage.setItem('moments_bg_image', e.target.result);
      this.renderMomentsTab();
