...
const MomentsEngine = {
  ...
  async persistMomentImage(moment) {
    if (!moment || !moment.image || !window.LovestoryImageDB) return moment;
    ...
    try {
      if (data.startsWith('data:') && typeof compressImage === 'function') {
        ...
        await window.LovestoryImageDB.put(imageId, data);
      } else if (typeof downloadAndStoreImage === 'function') {
        await downloadAndStoreImage(data, imageId);
      } else {
        await window.LovestoryImageDB.put(imageId, data);
      }
      return { ...moment, image: null, imageId };
    } catch(e) {
      console.warn('[MomentsPersistence] Failed to persist image asset:', e);
      return { ...moment, image: data && data.startsWith('data:') ? null : data, imageId };
    }
  },
  ...
  hydrateHeroImage() {
    ...
    if (!hero || !window.LovestoryImageDB) return;
    ...
    } else if (legacy && legacy.startsWith('data:')) {
      const newId = 'moments-hero-bg';
      window.LovestoryImageDB.put(newId, legacy).then(() => {
        localStorage.setItem('moments_bg_image_id', newId);
  ...
  async addMoment(moment) {
    ...
    if (moments.length > 100) {
      ...
      if (removed && window.LovestoryImageDB) {
        window.LovestoryImageDB.remove(removed.imageId || removed.id).catch(() => {});
      }
      this.saveMoments(moments);
    }
  },
...
2. 仅返回纯回复正文，不要有任何前言、后记、Markdown 或引号。`;
  ...
  handleUploadedImage(input) {
    ...
    r.onload = async e => {
      ...
      const compressed = (typeof compressImage === 'function') ? await compressImage(raw, 1280, 0.78) : raw;
  ...
  handleHeroImage(input) {
    ...
    r.onload = async e => {
      const raw = e.target.result;
      const compressed = (typeof compressImage === 'function') ? await compressImage(raw, 1600, 0.82) : raw;
      const bgId = 'moments-hero-bg';
      if (window.LovestoryImageDB) await window.LovestoryImageDB.put(bgId, compressed);
